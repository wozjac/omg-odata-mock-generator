const { JSDOM } = require( "jsdom" ); const { window } = new JSDOM("" ); const jQuery = require( "jquery" )( window );
'use strict';
    ...
    if (this._fakerConfig[entityType.name] &&
      ...
      try {
        if (fakerCall.indexOf("{{") !== -1) {
          generatedValue = faker__default['default'].fake(fakerCall);
        } else {
          const fakerCallParts = fakerCall.split(".");
          generatedValue = faker__default['default'][fakerCallParts[0]][fakerCallParts[1]].call();
        }
